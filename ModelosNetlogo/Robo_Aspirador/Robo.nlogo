;; This file was generated by the Behaviour Composer at modelling4all.org on Thu Jun 06 22:50:26 UTC 2019
;; The model can be found at http://m.modelling4all.org/m/?frozen=_vhfs0Au1ReBIIunxrY544
...
to the-model [ globals-not-to-be-initialised ] 
   ...
   create-objects 1 
                  [ set kind 
                    ...
                    initialise-object 
                    set hidden? true ] 
   ask all-of-kind "Ambiente Gerador" 
       [ -PEDREIRO-4 
         -CRIA-24 ] 
end

to kind-initialisation [ kind-name ] 
   if-else ( kind-name = "Roomba" ) 
           [ -JUMP-TO-UNOCCUPIED-LOCATION-IN-REGION-2040 
             -SHAPE-4139 
             -MOVIMENTO-59 
             -JUMP-14 ] 
           [ if-else ( kind-name = "Lixo" ) 
                     [ -JUMP-TO-UNOCCUPIED-LOCATION-IN-REGION-2041 
                       -SHAPE-4140 
                       -JUMP-15 ] 
                     [ if-else ( kind-name = "Ambiente Gerador" ) 
                               [ -PEDREIRO-4 
                                 -CRIA-24 ] 
                               [ output-print ( word "create-agent called with an unknown kind: " kind-name ) ] ] ] 
end

to -JUMP-TO-UNOCCUPIED-LOCATION-IN-REGION-2040
...
to -MOVIMENTO-59 
   do-every ( 1 ) 
            [ [] ->  let x0 
                   ...
                   ifelse ( any? patches with [ pcolor = green ] in-cone 1 90 ) or ( any? all-others with [ kind = "Roomba" ] in-cone 1 90 ) 
                          [ set heading 
                                ( heading + 5 * ( dir * 2 + 1 ) ) ] 
                          [ go-forward 0.1 ] 
                   ask all-others with [ kind = "Lixo" ] in-radius size 
                       [ die ] ] 
end
...
to -JUMP-TO-UNOCCUPIED-LOCATION-IN-REGION-2041
                                               ; Jump to a random unoccupied location in the specified region.  
   let unoccupied-location 
       random-unoccupied-location ( min-pxcor ) ( max-pxcor ) ( min-pycor ) ( max-pycor ) 
   set my-x 
...
to -CRIA-24 
   let just-created-agents 
       nobody 
   ask-every-patch [ [] ->  if ( pycor = min-pycor ) or ( pycor = max-pycor ) or ( pxcor = min-pxcor ) or ( pxcor = max-pxcor ) 
                             [ set pcolor 
                                   green ] ] 
   let qtd_paredes 
       random-integer-between 3 8 
   let x 
       random-integer-between ( min-pxcor + 1 ) ( max-pxcor - 1 ) 
   let y 
       random-integer-between ( min-pycor + 1 ) ( max-pycor - 1 ) 
   let comp_parede 
   ...
   let i 
       0 
   do-repeatedly qtd_paredes [ [] ->  ifelse ( vertical = 1 ) 
                                           [ do-repeatedly comp_parede 
                                           [ [] ->  ask patch x ( y + i ) 
                                                      [ set pcolor 
                                                            green ] 
                                                  set i 
                                                      i + 1 ] ] 
                                           [ do-repeatedly comp_parede [ [] ->  ask patch ( x + i ) y 
                                                                                  [ set pcolor 
                                                                                        green ] 
                                                                              set i 
                                                                                  i + 1 ] ] 
   ...
   ask just-created-agents 
       [ kind-initialisation "Roomba" ] 
   ask just-created-agents [ ]
       ; The following code was generated by a call to create-agent. 
   set just-created-agents 
       nobody 
   ask patch pxcor pycor 
       [ sprout-objects qtd_lixo [ 
                                   set just-created-agents 
                                       ( turtle-set self just-created-agents ) 
                                   set kind 
                                       "Lixo" ] ] 
